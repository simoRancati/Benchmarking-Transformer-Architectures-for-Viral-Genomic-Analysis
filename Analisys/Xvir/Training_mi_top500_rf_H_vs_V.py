...
# ===================== Logging =====================
logger = logging.getLogger("trainer_rf_giniFS")
...
def merge_and_clean(dfs):
    ...
    if not nonempty: return pd.DataFrame()
    combined = nonempty[0]
...
def _fit_rf_cv(Xtr, ytr, gtr, seed):
    ...
    gs = GridSearchCV(rf, params, cv=cvobj, scoring='balanced_accuracy', verbose=1, n_jobs=1, refit=True)
...
def evaluate_with_gini_fs(df_pipeline, train_ids, test_ids, seed, scenario_safe, embedding_tag, run_no):
    ...
    for k in TOPK_LIST:
        ...
        model, bestp = _fit_rf_cv(Xtr_k, ytr, gtr, seed)
