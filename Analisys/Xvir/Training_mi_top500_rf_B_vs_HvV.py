#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# RF with SelectKBest(mutual_info_classif) — Bacteria vs Human vs Virus — top500
...
KEEP_LABELS = "bacteria", "human", "virus"

CSV_CHUNKSIZE = 100_000
NUMERIC_DTYPE = np.float32

TOPK = 500   # None => all (k='all')
MODEL_NAME = "RF"
TOPK_TAG = "top500"
...
# ===================== Logging =====================
logger = logging.getLogger("RF_mi_top500")
...
def _fit_model_cv(Xtr, ytr, gtr, seed):
    ...
    gs = GridSearchCV(rf, params, cv=cvobj, scoring='balanced_accuracy',
...
def evaluate_with_mi_fs(df_pipeline, train_ids, test_ids, seed, scenario_safe, embedding_tag, run_no):
    ...
    if TOPK is None:
        ...
        selector, idx, sel_feat_names, eff_k, _scores = _corr_ratio_topk_from_train(Xtr, ytr, feat_cols, TOPK)
...
# === Multi-class Correlation Ratio (η²) ===

def _corr_ratio_topk_from_train(Xtr, ytr, feat_names, k, log_topn: int = 10):
    """
    Multi-classe: ranking per correlation ratio (eta^2) tra feature continua e classe.
    Restituisce: selector, idx, sel_feat_names, eff_k, scores (eta^2 in [0,1]).
    """
    t0 = perf_counter()
    y = np.asarray(ytr)
    n, p = Xtr.shape

    # codifica classi -> indici 0..C-1
    classes, y_codes = np.unique(y, return_inverse=True)
    C = len(classes)

    try:
        logger.info(f"[FS-ETA2] start | n={n} p={p} | k={k} | classes={C}")
    except Exception:
        pass

    # conteggi per classe
    counts = np.bincount(y_codes, minlength=C).astype(np.float64)
    counts[counts == 0] = 1.0  # sicurezza numerica (non dovrebbe servire)
    ...
    for j in range(p):
        ...
        # Imputazione minima per la sola stima dello score (se servisse)
        if not np.isfinite(x).all():
            finite_mask = np.isfinite(x)
            if not finite_mask.any():
                scores[j] = 0.0
                continue
            mean_finite = np.mean(x[finite_mask])
            x = np.where(finite_mask, x, mean_finite)
        ...
        # media per classe via somma pesata
        sum_per_cls = np.bincount(y_codes, weights=x, minlength=C)
        mean_per_cls = sum_per_cls / counts

        # between-class variance (S_B)
        sb = np.sum(counts * (mean_per_cls - mu) ** 2)

        # eta^2 = S_B / (N * var)
        eta2 = sb / (len(x) * var)
        if not np.isfinite(eta2) or eta2 < 0:
            eta2 = 0.0
        elif eta2 > 1.0:
            eta2 = 1.0
    ...
    dt = perf_counter() - t0
    try:
        logger.info(f"[FS-ETA2] done  | eff_k={eff_k} | time={dt:.2f}s")
        topn = min(log_topn, eff_k)
        preview = ", ".join(f"{sel_feat_names[i]}(η²={scores[idx[i]]:.4f})" for i in range(topn))
        logger.info(f"[FS-ETA2] top{topn}: {preview}")
    except Exception:
        pass
...
def train_on_saved_splits():
    ...
    for fn in per_run_files:
        ...
        for embedding_tag, df_pipe in (("XVir", df_xvir),):
            if df_pipe.empty:
