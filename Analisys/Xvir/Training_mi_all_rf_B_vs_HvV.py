#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# RF with SelectKBest(mutual_info_classif) — Bacteria vs Human vs Virus — all
...
# ===================== Logging =====================
logger = logging.getLogger("RF_mi_all")
...
def evaluate_with_mi_fs(df_pipeline, train_ids, test_ids, seed, scenario_safe, embedding_tag, run_no):
    ...
    if TOPK is None:
        # niente FS: usa direttamente Xtr, Xte
        Xtr_k, Xte_k = Xtr, Xte
        eff_k = Xtr.shape[1]
        sel_feat_names = feat_cols
    else:
        selector, idx, sel_feat_names, eff_k = _mi_topk_from_train(Xtr, ytr, feat_cols, TOPK, seed)
